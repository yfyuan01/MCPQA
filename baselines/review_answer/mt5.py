...
def T5Trainer(
    ...
    contents = [json.loads(i) for i in open(train_file).readlines()][:500]
    if country in country1:
        ...
        if args.gpt4:
            t_dict = pickle.load(open(os.path.join(text_address, 'translated_gpt_answer_' + c + '.pkl'),'rb'))
            contents = [i for i in contents if i['topAnswer'] != '']
            source_text_train = [i['question'] + ' ' + ' '.join(i['bm25_top5']) for i in contents]
            target_text_train = [t_dict[i['gpt4_answer']] for i in contents]
            contents = [json.loads(i) for i in open(val_file).readlines()]
            contents = [i for i in contents if i['topAnswer'] != '']
            source_text_val = [i['question'] + ' ' + ' '.join(i['bm25_top5']) for i in contents]
            target_text_val = [t_dict[i['gpt4_answer']] for i in contents]
        else:
            contents = [i for i in contents if i['topAnswer'] != '']
            source_text_train = [i['question'] +' '+ ' '.join(i['bm25_top5']) for i in contents]
            target_text_train = [i['topAnswer'] for i in contents]
...
def T5Tester(text_address, model_params, country, market_type, output_dir="./outputs/"):
    ...
    model = MT5ForConditionalGeneration.from_pretrained(os.path.join(output_dir,"model_files"))
    model = model.to(device)
    ...
    if country in country1:
        ...
        if args.gpt4:
            t_dict = pickle.load(open(os.path.join(text_address,'translated_gpt_answer_'+c+'.pkl'),'rb'))
            target_text_test = [t_dict[i['gpt4_answer']] for i in contents]
            source_text_test = [i['question'] + ' ' + ' '.join(i['bm25_top5']) for i in contents]
        else:
            target_text_test = [i['topAnswer'] for i in contents]
            source_text_test = [i['question'] + ' '+ ' '.join(i['bm25_top5']) for i in contents]
    ...
    if c=='cn':
        b_results = bleu.compute(predictions=predictions, references=[[r] for r in actuals], max_order=1, smooth=True,tokenizer=tokenizer_zh.TokenizerZh())
        b = b_results["bleu"]
        from rouge_chinese import Rouge
        import jieba
        cn_rouge = Rouge()
        predictions = [' '.join(jieba.cut(h)) for h in predictions]
        actuals = [' '.join(jieba.cut(h)) for h in actuals]
        r = cn_rouge.get_scores(predictions, actuals, avg=True)
    elif c=='jp':
        b_results = bleu.compute(predictions=predictions, references=[[r] for r in actuals], max_order=1, smooth=True,tokenizer=tokenizer_ja_mecab.TokenizerJaMecab())
        b = b_results["bleu"]
        avg_r = []
        from sumeval.metrics.rouge import RougeCalculator
        rouge_jp = RougeCalculator(stopwords=True, lang="ja")
        for i in range(len(predictions)):
            r = rouge_jp.rouge_l(summary=predictions[i], references=actuals[i])
            avg_r.append(r)
...
if args.gpt4:
    output_dir = "gpt4_t5_outputs_" + c + "_" + market_type
else:
    output_dir = "t5_outputs_"+c+"_"+market_type
if args.gpt4:
