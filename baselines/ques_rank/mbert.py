...
for c in country1:
    ...
    pre_file = os.path.join(test_file_address+'/intermediate',c+'_quesrank_'+market+'_market_exact.jsonl')
    ...
    for k,t in tqdm(enumerate(pre_relevance)):
        ...
        for records in original_ques:
            pairs.append((t['translatedQuestion'] + ' <SEP> ' + t['translatedAnswer'], records))  # topAnswer
        scores = model.predict(pairs)
        scores = softmax(scores, axis=1)
        ranked_index = list(np.argsort(scores, axis=0)[:,0])
...
for c in country:
    ...
    for k,t in tqdm(enumerate(pre_relevance)):
        ...
        original_ques = t['bm25_top5']
        for records in original_ques:
            pairs.append((t['question'] + ' <SEP> ' + t['topAnswer'], records))  # topAnswer
        scores = model.predict(pairs)
        scores = softmax(scores, axis=1)
        ranked_index = list(np.argsort(scores, axis=0)[:, 0])
        ranked_index.reverse()
