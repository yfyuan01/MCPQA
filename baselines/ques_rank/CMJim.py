...
for c in country1:
    print(c)
    model = SentenceTransformer('/home/dpl944/ProductQA/Ourdata/cmjim/trained_models/sbert_test_'+c)
    mrr_all = 0
    p_all = 0
    ques_true = {}
    ques_sent = {}
    ...
    pre_file = os.path.join(test_file_address+'/intermediate',c+'_quesrank_'+market+'_market.jsonl')
    ...
    for k,t in tqdm(enumerate(pre_relevance)):
        ...
        mrr = 0
        if market == 'single':
            original_ques = t['translated_bm25_top5']
        else:
            original_ques = t['bm25_top5']
        query_embedding = model.encode(t['translatedQuestion'] + ' <SEP> ' + t['translatedAnswer'])
        passage_embedding = model.encode(original_ques)
        scores = util.dot_score(query_embedding, passage_embedding)
        ranked_index = list(np.argsort(scores.numpy(), axis=1)[0])
        ranked_index.reverse()
        ranked_ques = [original_ques[l] for l in ranked_index]
        pre = [pp.split(' <SEP> ')[0] for pp in ranked_ques]
...
for c in country:
    ...
    for k,t in tqdm(enumerate(pre_relevance)):
        ...
        pairs = []
        original_ques = t['bm25_top5']
        query_embedding = model.encode(t['question']+ ' <SEP> ' + t['topAnswer'])
        passage_embedding = model.encode(original_ques)
        scores = util.dot_score(query_embedding, passage_embedding)
        ranked_index = list(np.argsort(scores.numpy(),axis=1)[0])
        ranked_index.reverse()
